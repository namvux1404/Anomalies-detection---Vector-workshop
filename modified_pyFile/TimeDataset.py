...
class TimeDataset(Dataset):
    def __init__(self, raw_data, edge_index, mode='train', config = None):
        ...
        if self.mode == 'train':
          train_data = np.transpose(np.array(data))
          scaler = MinMaxScaler(feature_range=(0, 1)).fit(train_data) # scale training data to [0,1] range
          data = scaler.transform(train_data)
          scaler_filename = "/content/Anomalies-detection---Vector-workshop/data/scaler.gz"
          joblib.dump(scaler, scaler_filename)        
        else:
          scaler_filename = "/content/Anomalies-detection---Vector-workshop/data/scaler.gz"
          test_data = np.transpose(np.array(data))
